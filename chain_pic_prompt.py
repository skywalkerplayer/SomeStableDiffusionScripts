...
class Script(scripts.Script):
    ...
    def run(self, p, put_at_start, different_seeds, prompt_type, variations_delimiter, margin_size):
        ...
        # combination_count = len(prompt_matrix_parts) + 1
        # for combination_num in range(combination_count):
        #     selected_prompts = [text.strip().strip(',') for n, text in enumerate(prompt_matrix_parts[1:]) if combination_num & (1 << n)]
        for i in range(p.batch_size):
            for j in range(p.n_iter):
                for text in prompt_matrix_parts[1:]:
                    selected_prompts = [text.strip().strip(',')]
        ...
        p.n_iter = (len(prompt_matrix_parts)-1)*p.n_iter
        p.do_not_save_grid = True

        print(f"Chain prompts painting will create {len(all_prompts)} images using a total of {p.batch_size} batches.")
        ...
        p.seed = [p.seed[i] + i/(len(prompt_matrix_parts)-1) for i in range(len(all_prompts))]
